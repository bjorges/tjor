...
The `[landlock]` config SHALL accept additional deny paths (`deny_paths`) that are applied as launch-time masks. Configuration SHALL only extend the default deny set, never weaken it.
