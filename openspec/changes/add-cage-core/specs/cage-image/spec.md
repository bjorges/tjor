...
The harness process and everything it spawns SHALL run as a dedicated non-root user inside the container.

#### Scenario: Identity check at start
- **WHEN** the container starts and the harness launches
- **THEN** the effective UID is the dedicated non-root user
